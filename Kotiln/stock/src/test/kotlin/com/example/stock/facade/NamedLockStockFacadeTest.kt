...
class NamedLockStockFacadeTest : RepositoryTestBase() {
    @Autowired
    ...
    fun `재고는 감소될 수 있다`() {
        ...
        // when
        namedLockStockFacade.decrease(stock.id, decreaseQuantity)
    ...
    fun `남은 재고보다 많이 감소될 수 없다`() {
        ...
        // when & then
        org.junit.jupiter.api.assertThrows<ClientBadRequestException> {
            namedLockStockFacade.decrease(stock.id, decreaseQuantity)
        }
            .also { throwable ->
    ...
    fun `동시에 여러 명이 주문해도 동시성 이슈가 발생하지 않는다`() {
        ...
        repeat((1..quantity).count()) {
            executorService.submit {
                try {
                    namedLockStockFacade.decrease(stock.id, 1)
                } finally {
                    latch.countDown()
