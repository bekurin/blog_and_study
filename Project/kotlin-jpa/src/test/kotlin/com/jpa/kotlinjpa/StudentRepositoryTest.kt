...
class StudentRepositoryTest {
    ...
    private val entityInformation: JpaEntityInformation<Student, *> = JpaEntityInformationSupport
    ...
    fun `저장 후 영속 상태 확인`() {
        ...
        println("현재 ID는 무엇인가? = ${student.id}")
        studentRepository.save(student)

        val findStudent = studentRepository.findById(student.id).get()
        println("현재 영속 상태인가? = ${entityInformation.isNew(findStudent)}")
        println("현재 ID는 무엇인가? = ${findStudent.id}")
